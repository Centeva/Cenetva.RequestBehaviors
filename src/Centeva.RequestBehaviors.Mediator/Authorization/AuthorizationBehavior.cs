...
namespace Centeva.RequestBehaviors.Mediator.Authorization;
    ...
    where TRequest : IMessage
{
    ...
    /// <summary>
    /// Processes the specified request by evaluating authorization requirements before invoking the next message
    /// handler in the pipeline.
    /// </summary>
    /// <param name="request">The request message to be handled. Must not be null.</param>
    /// <param name="next">A delegate representing the next handler to invoke if authorization succeeds.</param>
    ...
    /// if authorization is successful; otherwise, an unauthorized response.</returns>
